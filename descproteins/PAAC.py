...
def PAAC(fastas, lambdaValue=10, w=0.05, **kw):
    if check_sequences.get_min_sequence_length_1(fastas) < lambdaValue + 1:
        print(
            'Error: all the sequence length should be larger than the lambdaValue+1: ' + str(lambdaValue + 1) + str(check_sequences.get_min_sequence_length_1(fastas))+'\n\n')
        return 0
